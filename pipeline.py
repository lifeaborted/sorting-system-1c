...
@dataclass
class Detection:
    """Результат детекции одной области маркировки."""
...
class PipelineResult:
    """Итоговый JSON-результат обработки одного кадра."""
    timestamp: str
    source: str
    processing_time_ms: float
    detections: list[Detection] = field(default_factory=list)
    status: str = "ok"          # ok | no_detections | error
...
class MarkingPipeline:
    ...
    def process_image(self, image: np.ndarray, source: str = "frame") -> PipelineResult:
        """
        Обработать один кадр (np.ndarray BGR).

        Возвращает PipelineResult с полем detections.
        """
    ...
    def process_camera(
        ...
        """
        Захват с камеры в реальном времени.

        Пространство: нажать S — обработать текущий кадр,
                       Q / Esc — выйти.
        Если show_preview=False — работает headless (для сервера).
        """
        ...
        logger.info(f"Камера #{camera_index} открыта. S — обработать кадр, Q — выйти.")
        frame_id = 0

        while True:
            ret, frame = cap.read()
            if not ret:
                logger.warning("Пустой кадр, пропускаем.")
                continue

            # Аннотированный preview
            if show_preview:
                preview = frame.copy()
                cv2.putText(
                    preview, "S: захват | Q: выход",
                    (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2
                )
                cv2.imshow("MarkingPipeline — preview", preview)
            ...
            if key == ord("s"):
                frame_id += 1
                source_name = f"cam{camera_index}_frame{frame_id:04d}"
                result = self.process_image(frame, source=source_name)

                if save_json:
                    json_path = out_dir / f"{source_name}.json"
                    json_path.write_text(result.to_json(), encoding="utf-8")
                    logger.info(f"Сохранён: {json_path}")

                print(result.to_json())

                if show_preview:
                    annotated = self._draw_detections(frame, result)
                    cv2.imshow("MarkingPipeline — результат", annotated)
    ...
    def _draw_detections(image: np.ndarray, result: PipelineResult) -> np.ndarray:
        """Нарисовать bbox и текст на изображении (для отладки)."""
        img = image.copy()
